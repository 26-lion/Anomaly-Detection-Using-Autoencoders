...
import cv2
import os
model_encoder = load_model("auto-plant_final.h5")
model_classification = load_model("class-plant3_final.h5")
def preprocess(path):
    img = cv2.imread(path)
    img = img/255
...
def outcomes(img):
    ...
    loss = np.abs(img_reconstructed - img)
...
def pred():
    if request.method == "GET":
        ...
        if common_name == "error" and scientific_name == "error":
            return render_template("not_found.html")
        return render_template("index.html",file="static/"+a.filename, okay = result,common_name = common_name,scientific_name= scientific_name)
